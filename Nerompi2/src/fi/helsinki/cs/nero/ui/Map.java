 ...
 * py�rii l�hinn� Swingin tapahtumak�sittelij�s�ikeess�. T�ytyy siis
 ...
 *  free        vapaa
 *  noposts     ei sis�ll� yht��n ty�pistett�
...
public class Map extends JSVGCanvas implements NeroObserver {
	...
	public synchronized void updateObserved(int type) {
        ...
        // jos tapahtuma oli aktiivisen huoneen vaihto, vaihdetaan my�s kerros tarvittaessa
	...
	private void clearRoomStatus() {
		...
		while(it.hasNext()) {
			...
			if(el == null) {
				System.err.println("Huonetta " + room.getRoomNumber() + " ei l�ydy kartalta.");
				continue;
			}
	 ...
	 * @param changeFloor vaihdetaanko my�s kerrosta
	 */
	 ...
	 * @note Kerroksen vaihto kartalla sy� runsaasti muistia! T�m� saattaa olla Batikin bugi.
	 */
	...
	private class RoomClickHandler implements EventListener {
		public void handleEvent(Event evt) {
			...
			if(room == null) {
				// Klikattu huonetta, jota ei l�ydy tietokannasta.
				session.setStatusMessage("Huonetta " + id + " ei l�ydy tietokannasta.");
				return;
			}
			setActiveRoom(room);
		}
